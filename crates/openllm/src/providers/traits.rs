...
/// Dynamic model info returned from provider APIs
...
pub trait Provider: Send + Sync {
    ...
    /// Get provider metadata (static info)
    fn metadata(&self) -> ProviderMetadata;

    /// Fetch available models from the provider API
    /// Returns None if the provider doesn't support dynamic model listing
    async fn list_models(&self, api_key: Option<&str>) -> ProviderResult<Option<Vec<DynamicModelInfo>>> {
        // Default implementation returns None (use static default_models from metadata)
        let _ = api_key;
        Ok(None)
    }
