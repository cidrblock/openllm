...
static REGISTRY: Lazy<RwLock<HashMap<String, StoreDefinition>>> = Lazy::new(|| {
    ...
    // Note: "env" store removed - env vars are now read explicitly based on config
...
mod tests {
    ...
    fn test_builtin_stores_registered() {
        ...
        assert!(has_secret_store("keychain"));
    }
    ...
    #[test]
    fn test_create_keychain_store() {
        let store = create_secret_store("keychain").unwrap();
    ...
    fn test_get_store_definition() {
        let def = get_store_definition("memory").unwrap();
        assert_eq!(def.0, "memory");
