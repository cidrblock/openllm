"""OpenLLM gRPC Client implementation."""
...
try:
    from .openllm.v1 import service_pb2 as proto
    from .openllm.v1 import service_pb2_grpc as grpc_service
except ImportError:
    proto = None
    grpc_service = None
...
class OpenLLMError(Exception):
    """Base exception for OpenLLM client errors."""
    pass


class ConnectionError(OpenLLMError):
    """Failed to connect to daemon."""
    pass


class NotFoundError(OpenLLMError):
...
class OpenLLMClient:
    ...
    def __init__(
        ...
        Args:
            socket_path: Path to Unix socket (default: ~/.openllm/daemon.sock)
    ...
    def _get_openllm_dir() -> Path:
        ...
        if xdg_runtime:
            return Path(xdg_runtime) / "openllm"
        return Path.home() / ".openllm"
    
    @classmethod
    def _default_socket_path(cls) -> str:
        """Get the default socket path."""
        return str(cls._get_openllm_dir() / "daemon.sock")
    
    @classmethod
    def _pid_file_path(cls) -> str:
        """Get the PID file path."""
        return str(cls._get_openllm_dir() / "openllm.pid")
    ...
    async def connect(self) -> None:
        ...
        if grpc_service is None:
            raise OpenLLMError(
                "gRPC stubs not generated. Run: ./proto/generate.sh python"
            )
        
        try:
            self._channel = grpc.aio.insecure_channel(self._target)
            self._stub = grpc_service.OpenLLMStub(self._channel)
    ...
    def _ensure_connected(self) -> None:
        ...
        if self._stub is None:
            raise OpenLLMError("Not connected. Use 'async with OpenLLMClient()' or call connect()")
