...
export class ChatViewProvider implements vscode.WebviewViewProvider {
  ...
  /**
   * Public API: Refresh the model list
   * Call this when OpenLLM configuration changes
   */
  ...
  private _getHtmlContent(webview: vscode.Webview): string {
    ...
    // Get codicon CSS and font URIs (bundled in out/codicons)
    const codiconCssUri = webview.asWebviewUri(
      vscode.Uri.joinPath(this._extensionUri, 'out', 'codicons', 'codicon.css')
    );
    const codiconFontUri = webview.asWebviewUri(
      vscode.Uri.joinPath(this._extensionUri, 'out', 'codicons', 'codicon.ttf')
    );
    
    // Get marked library URI (bundled in out/lib)
    const markedUri = webview.asWebviewUri(
      vscode.Uri.joinPath(this._extensionUri, 'out', 'lib', 'marked.umd.js')
    );
    
    // Get highlight.js URIs (bundled in out/lib)
    const hljsUri = webview.asWebviewUri(
      vscode.Uri.joinPath(this._extensionUri, 'out', 'lib', 'highlight.min.js')
    );
    const hljsStyleUri = webview.asWebviewUri(
      vscode.Uri.joinPath(this._extensionUri, 'out', 'lib', 'vs2015.min.css')
    );
...
<head>
  ...
  <style>
    /* Override codicon font URL to use our bundled version */
    @font-face {
      font-family: "codicon";
      font-display: block;
